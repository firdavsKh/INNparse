...
# print(add_one(get_random_code()[2]))
# print(get_random_code())
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if(response.json().get('userInfo')!=None):
                ...
                # sql_query = f"INSERT INTO sma_stat_dep.tbl_tax_id (inn,upload_tstmp,required_update) VALUES('{get_random_code()[0]}',CURRENT_TIMESTAMP,'false')"
                # cursor.execute(sql_query)
                ...
                sql_query = f"""WITH first_insert AS (
                    ...
                    second_insert AS (
                        ...
                        SELECT first_table_id,'{person["Name"]}','{person["Fam"]}','{person["Full_nameK"]}','{person["pol"]}','{person["Date_roj"]}','{person["Citizenship"]}',CURRENT_TIMESTAMP
                        FROM first_insert
                    ),
                    third_insert AS (
                    ...
                    SELECT '{person["inspectionName"]}','{person["inspectionRamz"]}','{person["id_insp"]}','{person["adr_txt"]}','{person["adr_phone"]}',CURRENT_TIMESTAMP
                    FROM first_insert"""
